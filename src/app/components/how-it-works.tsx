...
export function HowItWorks() {
  return (
    <section
      id="how-it-works"
      className="relative w-full bg-sidebar px-4 py-20 md:px-6 md:py-28"
    >
      <div className="container mx-auto max-w-5xl">
        <header className="mb-12 space-y-4 text-left md:text-center">
          ...
          <p className="mx-auto max-w-3xl text-base text-muted-foreground md:text-lg">
            We combine thoughtful onboarding with automation that adapts to your
            process. No sprawling playbooks, just a clear cadence that your
            teams adopt immediately.
          </p>
        </header>
        <div className="relative border-l border-border/70 pl-8 md:pl-12">
          <div
            className="absolute left-0 top-2 bottom-2 w-px bg-gradient-to-b from-primary/40 via-border/70 to-transparent"
            aria-hidden
          />
          <div className="space-y-10">
            {steps.map(({ title, description, detail, icon: Icon }, index) => (
              <article
                key={title}
                className="relative rounded-3xl bg-background/80 p-6 shadow-lg shadow-primary/5 md:p-8"
              >
