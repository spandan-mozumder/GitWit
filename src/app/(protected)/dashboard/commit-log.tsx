...
const CommitLog = () => {
    ...
    const {data: commits, error, isLoading} = api.project.getCommits.useQuery({projectId}, {
        ...
        <Card className="animate-fade-in overflow-hidden border border-border/70 bg-card/70 shadow-lg shadow-primary/5">
            ...
            <CardContent className="pt-6">
                <ul className='space-y-6'>
                    {commits
                        .sort((a: { commitDate: Date }, b: { commitDate: Date }) => new Date(b.commitDate).getTime() - new Date(a.commitDate).getTime())
                        .map((commit: { id: string; commitDate: Date; commitAuthorAvatar: string; commitAuthorName: string; commitMessage: string; commitHash: string; summary: string }, commitIdx: number) => (
                            <li key={commit.id} className='relative flex gap-x-4 group'>
                                ...
                                <div className='flex-auto rounded-xl border border-border/60 bg-background/80 p-4 transition-all hover:border-primary/40'>
                                    ...
                                    <div className="space-y-2">
                                        <p className='font-medium text-foreground text-sm'>{commit.commitMessage}</p>
                                        <div 
                                            ...
                                            dangerouslySetInnerHTML={{
                                                __html: commit.summary
                                                    .replace(/(\*{2})(.*?)\1/g, '<strong class="text-foreground">$2</strong>')
                                                    .replace(/(['"`])(.*?)\1/g, '<code class="text-primary bg-primary/10 px-1 rounded">$2</code>'),
                                            }}
                                        />
                                    </div>
                                </div>
