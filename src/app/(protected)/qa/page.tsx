          ...
          {!isLoading && (!questions || questions.length === 0) && (
            <Card className="border-dashed border-border/70 bg-card/70">
              <CardContent className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <h3 className="mb-2 text-lg font-semibold">No archived questions</h3>
                <p className="text-sm text-muted-foreground max-w-sm">
                  Ask GitWit a question above and save the answer to build your knowledge base
                </p>
              </CardContent>
            </Card>
          )}
      ...
      {question && (
        <SheetContent className='sm:max-w-[85vw] flex flex-col p-0'>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6">
            ...
            <CodeRefrence filesRefrences={(question.filesRefrences ?? []) as {fileName: string, sourceCode: string, summary: string}[]}/>
          </div>
        </SheetContent>
      )}
    </Sheet>
